...
#CPK: But for this ^^ we needed to group by date bc cycles refer to each experiment, not across them. [-1]. And we needed to group by bl.s rather than speed to joing to met data later
...
#CPK: No need to do the SEM calculation separately. [-1] You could have defined the se function earlier and computed se at the same time as mean, a la.. . 

pseed.sum.max<- pseed.max %>%
  group_by(fish, speed) %>%
  summarize(amp.sum.mean=mean(amp.sum),amp.sum.se = stdErr(amp.sum))
...
#CPK: Why not color the error bars by fish? 
