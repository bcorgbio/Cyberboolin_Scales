...
First, an anole.log data tibble was established. 

Following this, two linear models using the log-transformed data from anole.log were constructed. These showed the effect of using perch height and diameter as covariates.
...
A phylogenetic least squares model of the hindlimb-SVL relationships **(hindlimb-SVL relationship + perch height, hindlimb-SVL relationship + perch diameter, and hindlimb-SVL relationship + perch height + perch diamete)** was then constructed using a BM (Brownian Motion) model and the phylogenetic tree by @poe2017phylogenetic

Then, the fit of each of the three previously mentioned models was assessed using AICc and AICw. AICc and AICw were also used to assess if any or both of the covariates were a predictor of hindlimb length.
...
## Discussion
The results of the PGLS models in addition to the analysis of the best fitting PGLS model, we decided that the PGLS under the Brownian Motion model including ecomorphs (PGLS.BM2) has the best fit. With this, we can come to the conclusion that ecomorphs have an impact on the relationship between snout-vent length and hindlimb length. Because the Brownian model has a better fit than the OU model, and because it is phylogenetically corrected, we can conclude that the traits evolved randomly, but also randomly within each lineage.
